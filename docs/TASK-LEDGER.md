...
### T-NOW-11　【今すぐ・吉本さん】AI（Anthropic）の残高が切れ、本番の点検・つくる・一式まとめてと自動レビューが止まっている（2026-09-25 11:40 確認）
- **優先**: P0 いま最優先　**担当**: 吉本さん　**状態**: 未着手　**大きさ**: 小(〜1時間)
- **止めているもの**: 本番の AI を使う機能すべて（職員の画面に英語の文「Your credit balance is too low…」がそのまま出る）。PR #24・#25 の自動レビュー（0秒で失敗）
- **次の一手**: console.anthropic.com → Plans & Billing でクレジットを購入する（Claude は購入の操作をしない）。購入後に Claude が本番の点検を1回通して復旧を確かめ、PR #24・#25 の自動レビューをやり直す
- **あわせて直すこと（Claude）**: 残高切れなど AI 側の失敗の文を、職員向けの日本語に置き換える（今は英語の文をそのまま出している ── app/api/evaluate/route.ts の AI の返事の失敗の扱い）
- <small>根拠（開発向け）: 手元の鍵で GET /v1/models → 200・POST /v1/messages（max_tokens 1）→ 400「Your credit balance is too low」。本番の点検（試験用 PDF）も同じ文。9:17 の本番の点検と 11:2x の PR #23 の自動レビューは成功していた＝その後に尽きた。3回目（07-16〜08-09・09-13・09-25）</small>
...
- **やったこと（2026-09-25）**: 実物と突き合わせて直した ── 説明書を v0.7 に（画面内録音は 2026-09-18 から使える状態・共有状態の表示は「画面いちばん上の帯の右側」で言葉は「事業所で共有中」「自分の登録分のみ」・冒頭の版表記 v0.5 の食い違い）、ADMIN-SETUP の切り替えの場所と言葉、録音の仕様書の R3・R5・環境変数の状態、拡張の「作成」→「つくる」。**残り**: 説明書の「削除に失敗した時は画面に警告」は、実は画面に出していなかった（救済モードはサーバーが警告を返すが画面が表示していない・点検はサーバーの記録だけ）→ 約束どおり画面に出す修正を PR #23 で入れた（2026-09-25・取り込み済み。資料は読み込んだ直後に消し、削除の失敗・時間切れを画面に黄色の警告で出す）。「評価する→点検」の言い換えと、再配布前の吉本さんの差分確認。
