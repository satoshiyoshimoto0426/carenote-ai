...
- **優先**: P1 次にやる　**担当**: Claude　**状態**: 未着手　**大きさ**: 小(〜1時間)
- **次の一手**: 「大阪市北区梅田1-2-3」を先に失敗するテストとして書き、市区町村始まりの住所も拾う規則を lib/privacy/patterns.ts に足す（漏れ検査 hasLongDigitRun とは別系統のまま）。
- <small>根拠（開発向け）: lib/privacy/patterns.ts:73 コメント「都道府県から番地（数字を含む）まで」／テスト（maskBody.test.ts:36・maskPii.test.ts:12・patterns.test.ts:32・vault.test.ts:10）はすべて「大阪府」付きのみ／`gh issue list` → #8 OPEN</small>
