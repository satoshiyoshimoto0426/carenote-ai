...
> **2026-09-11 追記**：アセスメント11ページ全欄・第2表の画面往復・使えない文字・onkeyup 同期・登録エラーの回避策は、
> ブラウザ操作型AIによる実機転記（2026-09-09〜10）の記録 [KAIPOKE-TRANSCRIPTION-SPEC.md](KAIPOKE-TRANSCRIPTION-SPEC.md) が最新。
> 本書の P1／P10・第4表・第5表・モニタリングの記述はそれと整合している（P1 の 26字幅を追記、P2 `form:supportSubject` を追加）。
