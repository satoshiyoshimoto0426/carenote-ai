...
**名簿の範囲（2026-09-12・G3b 吉本さん決定「事業所で共有」）**: `lib/db/clients.ts` の `scopeExpr` が唯一の絞り込み。組織に所属していれば「事業所の行＋組織加入前の自分の行」、していなければ本人のみ（`org_id` が null 同士は同じ事業所とみなさない）。利用者・実名・関係者の3表すべてがこれを通り、ルート9本が範囲を渡すことは `tests/api/orgScope.route.test.ts` が縛る。記号（A様）の採番は**範囲内の最大＋1**（件数だと範囲が混ざったとき同じ記号を二度振る）。安全網 `assertCodesUnique` が「同じ記号に違う氏名」を見つけたら送信を止め、名簿が900件の上限に達しても止める（PostgREST の既定1000行の黙った打ち切り対策）。**有効化には Clerk の組織設定＋SQL 2本の実行＋既存データの移行（`supabase/client_org_scope.sql` 手順2→3→4）が要る**。保存書類の共有は未対応。
