/**
 * 印刷・PDF配布用マニュアルのHTMLを組み立てる（純粋関数・副作用なし）。
 *
 * なぜ lib に置くか:
 *   本文の正本 lib/manual/content.ts と同じ層に置き、CLI（tools/build-manual.mjs）は
 *   「呼んでファイルに書くだけ」にする。テスト（manualHtml.test.ts）はこの関数を直接呼ぶ。
 *
 * 注意: 生成物 public/manual/index.html は手で直さない。本文を直して `npm run manual` で作り直す。
 */
// 型だけを取り込む（実行時の import を持たないので、Node からも Next からも同じように読める）
import type { ManualCalloutKind, ManualChapter } from "./content";

/** MANUAL_META と同じ形（as const の読み取り専用オブジェクトを受けられるように） */
...
const CALLOUT_LABEL: Record<ManualCalloutKind, string> = {
  info: "知っておくこと",
  warn: "気をつけること",
  tip: "こつ",
};

function chapterHtml(ch: ManualChapter): string {
...
export function buildManualHtml(
  chaptersData: readonly ManualChapter[],
  meta: ManualMeta,
  promisesData: readonly string[],
): string {
